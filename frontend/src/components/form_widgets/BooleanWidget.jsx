 ...
 */
const BooleanWidget = React.memo(({ 
    name, 
    value, 
    onChange, 
    darkMode, 
    ...
    return (
        <div className={cn(
            ...
            isImmutable ? (darkMode ? "bg-slate-900 border-slate-800" : "bg-slate-200 border-slate-300") : (isEnabled ? "border-blue-500/50 bg-blue-500/5" : inputBg),
            hasError && "border-rose-500 ring-1 ring-rose-500/50",
            isImmutable && "cursor-not-allowed opacity-80"
        )} onClick={() => !isImmutable && onChange(name, !isEnabled)}>
            <div className={cn(
                "w-10 h-6 rounded-full relative transition-colors p-1",
                isEnabled ? (isImmutable ? "bg-blue-600/50" : "bg-blue-600") : (isImmutable ? "bg-slate-800" : "bg-slate-700")
            )}>
                <div className={cn(
