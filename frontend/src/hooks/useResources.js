...
export const useDoris = () => {
    ...
    const fetchPlatforms = useCallback(async () => {
        setLoading(true);
        try {
            const response = await apiClient.get('/platform/doris');
            setPlatforms(response.data.data || []);
        } catch (err) {
            setError(err);
        } finally {
    ...
    const createPlatform = useCallback(async (data) => {
        const response = await apiClient.post('/platform/doris', data);
        await fetchPlatforms();
        return response.data;
    }, [fetchPlatforms]);

    const updatePlatformState = useCallback(async (id, state, headers = {}) => {
        const response = await apiClient.post(`/platform/doris/${id}/_state`, state, { headers });
        await fetchPlatforms();
        return response.data;
    }, [fetchPlatforms]);

    const deletePlatform = useCallback(async (id, confirmName) => {
        await apiClient.delete(`/platform/doris/${id}`, {
            headers: { 'X-RESOURCE-NAME': confirmName }
        });
        await new Promise(resolve => setTimeout(resolve, 500));
        await fetchPlatforms();
    ...
    const getPlatformState = useCallback(async (id) => {
        const response = await apiClient.get(`/platform/doris/${id}/_state`);
        return response.data;
    }, []);

    const refreshPlatformState = useCallback(async (id) => {
        const response = await apiClient.post(`/platform/doris/${id}/_refresh`);
        return response.data;
    }, []);

    const fetchActions = useCallback(async (id) => {
        const response = await apiClient.get(`/platform/doris/${id}/_actions`);
        return response.data.actions || [];
    }, []);

    const executeAction = useCallback(async (id, action, parameters = {}) => {
        const response = await apiClient.post(`/platform/doris/${id}/_actions`, { action, parameters });
        return response.data;
    }, []);
