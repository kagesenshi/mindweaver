...
const ListingView = ({
    ...
    const { platforms, loading, fetchPlatforms, getPlatformState, deletePlatform } = zookeeper;
    ...
    return (
        <GenericListingView
            title="ZooKeeper"
            description="Manage ZooKeeper coordination clusters for distributed services"
            items={platforms}
            loading={loading}
            ...
            icon={Network}
            entityPath="/platform/zookeeper"
            createConfig={{
                title: "Provision ZooKeeper",
                buttonText: "NEW ZOOKEEPER",
            }}
            searchPlaceholder="Search ZooKeeper clusters..."
            emptyState={{
                title: "No ZooKeeper clusters found",
                description: selectedProject ? `No ZooKeeper clusters in ${selectedProject.name}.` : 'Create your first ZooKeeper cluster to get started.',
                icon: <Network size={48} className="text-slate-700" />
            }}
            renderSubtitle={renderSubtitle}
            renderBadges={renderBadges}
            deleteModalConfig={{
                title: "Delete ZooKeeper",
                message: "Are you sure you want to decommission this ZooKeeper cluster? This will permanently delete the metadata and associated resources. Ensure no services depend on this cluster before proceeding."
            }}
            darkMode={darkMode}
