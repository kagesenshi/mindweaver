...
const ListingView = ({ darkMode, selectedProject, onSelectPlatform, doris }) => {
    const { platforms, loading, fetchPlatforms, getPlatformState, deletePlatform } = doris;
    ...
    return (
        <GenericListingView
            title="Apache Doris"
            description="Real-time analytical database and MPP data warehouse"
            items={platforms}
            loading={loading}
            ...
            createConfig={{
                title: "Provision Doris Cluster",
                buttonText: "NEW CLUSTER",
            }}
            searchPlaceholder="Search Doris clusters..."
            emptyState={{
                title: "No clusters found",
                description: selectedProject ? `No Doris clusters in ${selectedProject.name}.` : 'Create your first Doris cluster to get started.',
                icon: <Database size={48} className="text-slate-700" />
            }}
            renderSubtitle={renderSubtitle}
            renderBadges={renderBadges}
            deleteModalConfig={{
                title: "Delete Doris Cluster",
                message: "Are you sure you want to delete this Doris cluster? This will permanently remove all FE/BE nodes and associated storage."
            }}
            darkMode={darkMode}
