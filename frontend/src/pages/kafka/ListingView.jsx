...
import { RefreshCcw, Briefcase, Activity } from 'lucide-react';
...
const ListingView = ({
    ...
    const { platforms, loading, fetchPlatforms, getPlatformState, deletePlatform } = kafka;
    ...
    return (
        <GenericListingView
            title="Apache Kafka"
            description="Manage Apache Kafka event streaming clusters"
            items={platforms}
            loading={loading}
            ...
            createConfig={{
                title: "Provision Apache Kafka",
                buttonText: "NEW KAFKA",
            }}
            searchPlaceholder="Search Apache Kafka clusters..."
            emptyState={{
                title: "No Apache Kafka clusters found",
                description: selectedProject ? `No Apache Kafka clusters in ${selectedProject.name}.` : 'Create your first Apache Kafka cluster to get started.',
                icon: <RefreshCcw size={48} className="text-slate-700" />
            }}
            renderSubtitle={renderSubtitle}
            renderBadges={renderBadges}
            deleteModalConfig={{
                title: "Delete Apache Kafka",
                message: "Are you sure you want to decommission this Apache Kafka cluster? This will permanently delete the metadata and associated resources. Ensure no services depend on this cluster before proceeding."
            }}
            darkMode={darkMode}
