...
import { Search, Briefcase, Activity } from 'lucide-react';
...
const ListingView = ({
    ...
    const { platforms, loading, fetchPlatforms, getPlatformState, deletePlatform } = opensearch;
    ...
    return (
        <GenericListingView
            title="OpenSearch"
            description="Manage search, analytics, and visualization with OpenSearch"
            items={platforms}
            loading={loading}
            ...
            createConfig={{
                title: "Provision OpenSearch",
                buttonText: "NEW OPENSEARCH",
            }}
            searchPlaceholder="Search OpenSearch instances..."
            emptyState={{
                title: "No OpenSearch instances found",
                description: selectedProject ? `No OpenSearch instances in ${selectedProject.name}.` : 'Create your first OpenSearch instance to get started.',
                icon: <Search size={48} className="text-slate-700" />
            }}
            renderSubtitle={renderSubtitle}
            renderBadges={renderBadges}
            deleteModalConfig={{
                title: "Delete OpenSearch",
                message: "Are you sure you want to decommission this OpenSearch instance? This will permanently delete the metadata and associated resources."
            }}
            darkMode={darkMode}
