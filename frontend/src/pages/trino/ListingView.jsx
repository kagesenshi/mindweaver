...
import { Server, Briefcase, Activity } from 'lucide-react';
import GenericListingView from '../../components/GenericListingView';
import { cn } from '../../utils/cn';

const ListingView = ({ darkMode, selectedProject, onSelectPlatform, trino }) => {
    ...
    return (
        <GenericListingView
            ...
            description="Distributed SQL query engine managed on Kubernetes"
            items={platforms}
            loading={loading}
            ...
            createConfig={{
                title: "Provision Trino Cluster",
                buttonText: "NEW CLUSTER",
            }}
            searchPlaceholder="Search Trino clusters..."
            emptyState={{
                title: "No clusters found",
                description: selectedProject ? `No Trino clusters in ${selectedProject.name}.` : 'Create your first Trino cluster to get started.',
                icon: <Server size={48} className="text-slate-700" />
            }}
            renderSubtitle={renderSubtitle}
            renderBadges={renderBadges}
            deleteModalConfig={{
                title: "Delete Trino Cluster",
                message: "Are you sure you want to delete this Trino cluster? This will permanently remove all catalogs and associated resources."
            }}
            darkMode={darkMode}
