...
@pytest.mark.asyncio
async def test_solr_password_generation(mock_service_dependencies):
    """Test that admin_password is autogenerated during creation if not provided"""
...
def test_solr_get_internal_host():
    """Test that get_internal_host returns the service URL with port 8983."""
    model = SolrPlatform(name="solr-test", project_id=1)
    
    # 1. State is None
    host = SolrPlatformService.get_internal_host(model, None, "test-ns")
    assert host == "solr-test-solrcloud-headless.test-ns.svc.cluster.local:8983"

    # 2. State has extra_data with service_name
    class MockState:
        extra_data = {"service_name": "custom-solr-service-headless"}
    host = SolrPlatformService.get_internal_host(model, MockState(), "test-ns")
    assert host == "custom-solr-service-headless.test-ns.svc.cluster.local:8983"
