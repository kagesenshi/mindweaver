...
def test_project_form_views(client: TestClient):
    ...
    assert "immutable_fields" in data
    assert "internal_fields" in data

    # Project has no relationships to other registered services in its create form usually
    # But it has immutable fields
    assert "name" in data["immutable_fields"]
    assert "id" in data["internal_fields"]
...
def test_k8s_cluster_form_views(client: TestClient):
    ...
    assert "type" in data["widgets"]
    assert data["widgets"]["type"]["type"] == "select"
    assert "options" in data["widgets"]["type"]
    options = data["widgets"]["type"]["options"]
    ...
    assert "project_id" in data["widgets"]
    assert data["widgets"]["project_id"]["type"] == "relationship"
    assert data["widgets"]["project_id"]["endpoint"] == "/api/v1/projects"
...
def test_ai_agent_form_views(client: TestClient):
    ...
    assert "knowledge_db_ids" in data["widgets"]
    assert data["widgets"]["knowledge_db_ids"]["type"] == "relationship"
    assert data["widgets"]["knowledge_db_ids"].get("multiselect") is True
    assert data["widgets"]["knowledge_db_ids"]["endpoint"] == "/api/v1/knowledge_dbs"
