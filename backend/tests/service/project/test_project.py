...
def test_project_scoping(client: TestClient, test_cluster: dict):
    ...
    resp_p1 = client.get("/api/v1/database-sources", headers=headers_p1)
    ...
    resp_p2 = client.get("/api/v1/database-sources", headers=headers_p2)
    assert resp_p2.status_code == 200
    recs_p2 = resp_p2.json()["data"]
    assert len(recs_p2) == 1
    assert recs_p2[0]["name"] == "ds2"

    resp_all = client.get("/api/v1/database-sources")
